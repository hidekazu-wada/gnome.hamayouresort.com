...
// ご利用の流れの実データ待ち（ダミー）
//
// Hobie ペダルボード と カヤックレンタル & ツアー の2件だけ、主催が外部
// （HOBIE JAPAN / 5LAKES & MT）で現行サイトに「ご利用の流れ」の記載がない。
// 他の項目は実データが揃っているので、残るダミーはこの流れだけ。
// docs/現場ヒアリングシート.md に確認項目として起票済み。
// ---------------------------------------------------------------------------

const dummyLine = "ここに説明文が入ります。ここに説明文が入ります。";

const dummySteps: ActivityDetail["steps"] = [
