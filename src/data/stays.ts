...
const notes = [
  ...
  ["ホテルでは日帰り入浴を行っておりません。"],
  // ※ この「15時」は、なっぷのプラン41件の IN が全件「〜15:00」なのと整合している。
  //    施設概要欄だけが「13:00〜16:00」と書かれているが、そちらが古いとみて 15:00 を採用した
  [
...
// サイト概要のテーブル。
//
// チェックイン / チェックアウト / 車両乗入は 2026-08-04 に実データへ差し替えた。
// 出典はなっぷ https://www.nap-camp.com/yamanashi/13592
// （現状ここが唯一の販売窓口なので、数字が割れたらなっぷを正とする方針）。
//
// なっぷ上でサイト種別ごとにチェックイン時刻が違う（「※ゲル、キャビンは14:00～」）ため、
// 区画・フリーサイト用の specSite と、ゲル・トレーラー用の specCabin に分けている。
// 分ける前は全8種で1つの配列を共有しており、ゲルにも区画サイトの時刻が出ていた。
//
// ※ チェックインの終わりは 2026-08-04 に 16:00 → 15:00 へ修正した。
//   なっぷの施設概要欄は「13:00~16:00」だが、**プラン41件は全件 IN が「13:00～15:00」**
//   （ゲル・トレーラーは「14:00～15:00」）で、注意事項の「15時までにご連絡ください」とも
//   整合する。実際に予約できる時刻はプラン側なので、そちらを正とした。
// ※ 現行サイト https://www.hamayouresort.com/gnome/site-price/ は
//   「全宿泊 14:00 IN / 11:00 OUT」と書かれていて一見食い違うが、
//   あちらはゲル・トレーラー中心のページなので「種別で違う」と読めば整合する。
// ※ 朝食・夕食・地面はFigmaのダミーのまま（未確認）。とくに specCabin の
//   「土」は区画サイトの値をそのまま引き継いだだけで実態とは違うはず。
//   現場ヒアリングで確定させること。
// ※ 広さは 2026-08-05 に、なっぷの区画一覧に実寸がある4種（林間・区画・グループ・
//   ダブル）だけ暫定反映した（下の sizeLinesByKey）。区画ごとに寸法が違うため
//   「※区画により異なります」を添えている。湖畔・おまかせ・ゲル・トレーラーは
//   なっぷに記載がなく従来のダミー値のまま。代表値はヒアリングシート B-6③ で確認中。

// 車両乗入。もとは "可　※リードまたは、ケージをご利用ください" とペットの文言が
// 誤って入っていた欄。なっぷの記載に差し替えたもの
const carSpec = {
  label: "車両乗入",
  lines: [
    "可　※1サイトにつき1台まで",
    "※ワイドサイト・ダブルサイトなどは2台まで",
    "※追加の1台は駐車場（550円）をご利用ください",
    "※荷物の搬入・搬出時は一時的にサイト付近へ留め置き可能です",
  ],
};

/** 区画・フリーサイト（林間・湖畔・グループ・ダブル・おまかせ・区画） */
const specSite = [
  {
    label: "広さ",
    lines: [
...
const specCabin = [
  ...
  { label: "チェックイン", lines: ["14:00〜15:00　※アーリーチェックインは当日受付のみ可能"] },
  { label: "チェックアウト", lines: ["〜11:00"] },
  carSpec,
  { label: "朝食", lines: ["なし"] },
  { label: "夕食", lines: ["なし"] },
  { label: "地面", lines: ["土　※推奨ペグ：スチール,ネイル"] },
...
// 種別ごとの「広さ」欄の上書き（なっぷの区画一覧の実寸・2026-08-05暫定）。
// 2行目の想定説明はFigmaの文言をそのまま残している。
// ここに無い種別（湖畔・おまかせ・ゲル・トレーラー）は specSite / specCabin の
// 既定値（13.3m×6.5m）が出る。実寸確定時はここを差し替える。
const sizeLinesByKey: Record<string, string[]> = {
  rinkan: [
    "13.3m×6.5m　※区画により異なります",
    "車両１台、タープ1張り、20㎡までのテント1張りを想定した広さ",
  ],
  kukaku: [
    "9m×12.5m　※区画により異なります",
...
const base = [
  ...
  },
  {
    ...
    code: "A-1",
    badgeLines: ["最安"],
    // 定員はなっぷの「定員3名」に合わせた（Figmaのダミーは5名）。
    // 3名なので絞り込みの「5〜9名」からは外す
    tags: ["フリーサイト", "最大3名", "林間", "ACなし", "車両乗入◯", "ペット◯", "キャンピングカー×"],
    price: "¥3,300〜",
    filters: {
      type: ["フリーサイト"],
      capacity: ["〜4名"],
  ...
  },
  {
    ...
    badgeLines: ["ママに", "大人気"],
    badgeSize: "md",
    // 定員はなっぷの「定員8名」に合わせた（Figmaのダミーは5名）。絞り込みは変更なし
    tags: ["区画サイト", "最大8名", "林間", "ACあり", "車両乗入×", "ペット×", "キャンピングカー×"],
    price: "¥19,800〜",
    filters: {
      type: ["ゲルテント"],
      capacity: ["〜4名", "5〜9名"],
...
export const stays: Stay[] = Array.from({ length: 16 }, (_, i) => {
  ...
  return {
    ...
    heroPhotos,
    heroBody,
    spec: (key === "gel" || key === "trailer" ? specCabin : specSite).map(
      (row) =>
        row.label === "広さ" && sizeLinesByKey[key]
